...
object CorefAnnotation extends AnnotationMethod {
  ...
  def serialize(doc:Document, serDoc:DocumentBuilder):DocumentBuilder = serDoc.addCompound(protoCompoundGroup.setType(annotationType).setMethodIndex(_methodIndex)
    .addAllCompound(doc.coref.entities.map { entity =>
    protoCompound.setText(if(entity.canonicalName==null) entity.mentions.head.phrase.string else entity.canonicalName).addAllSlot(entity.mentions.map { mention =>
